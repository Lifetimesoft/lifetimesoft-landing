...
<body>
  ...
  <script>
    ...
    (function collabInit() {
      ...
      type PktRoute = { el: string; x1:number; y1:number; x2:number; y2:number; delay:number };
      const routes: PktRoute[] = [
      ...
      routes.forEach(({ el, x1, y1, x2, y2, delay }) => {
        const circle = document.querySelector(el) as SVGCircleElement | null;
        if (!circle) return;
        const t = { v:0 };
        gsap.to(t, {
